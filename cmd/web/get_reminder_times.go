...
// Formats start & end times of work events for use in cron jobs.
func formatWorkTimes() []string {
	workEvents := getWorkEvents()
	reminderIntervals := make([]string, 0)

	for _, event := range workEvents {
		hourStart, _, _ := event.start.Clock()
		hourEnd, _, _ := event.end.Clock()

		interval := strconv.Itoa(hourStart) + "-" + strconv.Itoa(hourEnd)

		reminderIntervals = append(reminderIntervals, interval)
	}
